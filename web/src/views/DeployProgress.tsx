...
/** 卸载流程的四步：连接 / 上传 / 卸载 / 校验，与后端 step 一一对应。 */
const UNINSTALL_STEPS: { key: DeployStep; label: string }[] = [
  { key: "connecting", label: "连接" },
  { key: "uploading", label: "上传" },
  { key: "uninstalling", label: "卸载" },
  { key: "verifying", label: "校验" },
];

/**
 * 部署/卸载进度：纯展示组件，只从模块级 deployStore 读取状态。
 * WebSocket 连接与状态都由 deployStore 持有，因此本组件的挂载/卸载
 * （即对话框的打开/关闭）不会影响进行中的操作——关闭对话框后它继续。
 * 重新打开时会看到累积日志与当前步骤，而非空白面板。
 */
export function DeployProgress() {
  let logEl: HTMLDivElement | undefined;

  const isUninstall = () => deployStore.action() === "uninstall";
  const steps = createMemo(() => (isUninstall() ? UNINSTALL_STEPS : DEPLOY_STEPS));
  const stepIndex = (step: DeployStep) =>
  ...
  return (
    <div class="deploy-progress">
      ...
      <div
        ...
        role="log"
        aria-live="polite"
        aria-label={isUninstall() ? "卸载日志" : "部署日志"}
      >
        <Show
